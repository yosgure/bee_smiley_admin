# プロジェクトルール

## デプロイ

- `firebase deploy --only hosting` を実行する前に、必ず現在のブランチの変更をmainブランチにマージすること
- mainブランチからビルド (`flutter build web`) してからデプロイすること
- 別ブランチからデプロイすると、他のセッションの変更が消える原因になる

## 開発

- 作業開始前に `git pull` を実行すること（最新の変更を取り込む）
- コミット後はmainにマージしてからデプロイする流れを守ること
- PRは作成しないこと（コミットのみ行う）。マージとデプロイは専用セッションが行う

## 動作確認

- UI変更の実装が完了したら、ユーザーに聞かずに自動的にローカルdevサーバーを起動すること
- コマンド: `flutter run -d chrome --web-port=<ポート番号>`
- ポート番号はセッションごとに変える（8080, 8081, 8082...）、既に使われている場合は別のポートを使う
- サーバー起動後、MCPのpreview_*ツールまたはClaude in ChromeのMCPツールを使って自律的に動作確認すること
- スクリーンショットを撮って変更が正しく反映されているかClaudeが自分で確認すること
- ユーザーに「確認してください」と求めないこと。Claudeが自分で確認して結果を報告すること
- MCPツールが使えない場合のみ、ユーザーに「localhost:<ポート番号> で確認してください」と伝えること
- 本番環境 (bee-smiley-admin.web.app) での確認はデプロイ後のみ
- firebase deploy は絶対に実行しないこと（デプロイは専用セッションが行う）

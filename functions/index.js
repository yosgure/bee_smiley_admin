...
// ==========================================
// モニタリング下書き新規作成 (未作成時のみ・安全ガード多重)
// ==========================================

/**
 * モニタリング一覧ページをスクレイプし、最新 kaisuu 行の状態を返す。
 */
...
/**
 * モニタリング INSERT フォームを取得してフォーム構造を返す。
 * id=insert の確認を行う。
 */
...
/**
 * モニタリング下書き保存 (INSERT のみ・多重安全ガード)
 *
 * request.data:
 *   - studentId: 必須
 *   - considerations: 目標行ごとの考察文字列配列（goals と同じ順・同じ長さ必須）
 *   - longTerm: 長期目標考察
 *   - shortTerm: 短期目標考察
 *   - remark: 備考欄
 */
exports.createMonitoringDraft = onCall(
  ...
  async (request) => {
    ...
    // ガード3: 目標行が 1件以上、かつ considerations と長さが一致
    ...
    // ガード4: フォーム既存の consideration が全て空であること（既に何か入っていたら中止）
    ...
    merged.created_name = '10'; // フィリップスヒロコ
